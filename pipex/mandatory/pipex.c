...
pid_t	ft_excute_1(char **split, char *argv[], int fd[2], char *envp[])
{
	char	*cmd_path;
	int		n;
	...
	if (n == -1 || dup2(n, 0) == -1)
		return (perror("Error: input file not found"), -1);
	close(n);
	close(fd[0]);
	close(fd[1]);
...
pid_t	ft_excute_2(char **split, char *argv[], int fd[2], char *envp[])
{
	char	*cmd_path;
	int		n;
	...
	if (n == -1 || dup2(n, 1) == -1)
		return (perror("Error: output file not found"), -1);
	close(n);
	close(fd[0]);
	close(fd[1]);
	cmd_path = ft_find_path(split[0], envp);
...
pid_t	child_process(int fd[2], char *argv[], char *envp[], int process)
{
	...
	if (pid == 0)
	{
		if (process == 1)
		{
			if (ft_excute_1(split, argv, fd, envp) == -1)
				return (freeall(split, countwords(argv[2], " \t")), -1);
		}
		else if (process == 2)
		{
			if (ft_excute_2(split, argv, fd, envp) == -1)
				return (freeall(split, countwords(argv[3], " \t")), -1);
		}
	}
	return (pid);
}
