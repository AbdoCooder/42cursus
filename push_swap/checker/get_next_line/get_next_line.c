...
/*   Updated: 2025/01/17 20:27:19 by abenajib         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (ft_strlen_gnl(sbuff, '\n') == -1)
		read_join(fd, &sbuff);
	line = ft_substr_gnl(sbuff, 0, ft_gnlstrlen2_gnl(sbuff, '\n') + 1);
	if (!line)
		return (free(line), free(sbuff), sbuff = NULL, NULL);
	tmp2 = sbuff;
	num = ft_gnlstrlen2_gnl(sbuff, '\n') + 1;
	sbuff = ft_substr_gnl(sbuff, num, ft_gnlstrlen2_gnl(sbuff, '\0'));
	free(tmp2);
	return (line);
}
// #include <stdio.h>
